...
function updateFn(f){
	if(f.id.value.trim() ==""){
		alert("수정하려면 아래의 리스트에서 수정할 데이터의 수정버튼을 눌러주세요.");
		return false;
	}
	return validateInput(f);
}
...
$(".bt-up").click(function(){
	$("form[name='updateForm']").find("input[name='stdname']").prop("disabled",false); //프로퍼티
	$("form[name='updateForm']").find("input[name='kor']").prop("disabled",false); //프로퍼티
	$("form[name='updateForm']").find("input[name='math']").prop("disabled",false); //프로퍼티
	$("form[name='updateForm']").find("input[name='eng']").prop("disabled",false); //프로퍼티
